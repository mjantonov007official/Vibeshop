...
Route::middleware('guest')->group(function (): void {
    ...
    Route::post('/login', [HomeController::class, 'customerLoginStore'])
    ...
    Route::post('/admin/login', [HomeController::class, 'adminLoginStore'])
        ->middleware('throttle:5,1')
        ->name('admin.login.store');
    Route::get('/forgot-password', [HomeController::class, 'forgotPassword'])->name('password.request');
    Route::post('/forgot-password', [HomeController::class, 'forgotPasswordStore'])
        ->middleware('throttle:3,1')
        ->name('password.email');
    Route::get('/reset-password/{token}', [HomeController::class, 'resetPassword'])->name('password.reset');
    Route::post('/reset-password', [HomeController::class, 'resetPasswordStore'])->name('password.store');
});
...
Route::get('/order-success', [HomeController::class, 'orderSuccess'])->middleware('auth')->name('order.success.empty');
Route::get('/order-success/{order:reference}', [HomeController::class, 'orderSuccess'])->middleware('auth')->name('order.success');
